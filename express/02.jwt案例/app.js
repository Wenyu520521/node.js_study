...
// 在 Express 4.16.0 版本之前，解析 POST 请求需要手动安装和使用 body-parser。而在 Express 4.16.0 版本之后，Express 内置了 express.urlencoded() 中间件，用于解析 POST 请求中的表单数据，因此不再需要手动安装 body-parser。
// 解析 post 表单数据的中间件
...
const secretKey = 'zxlml'

// TODO_04：注册将 JWT 字符串解析还原成 JSON 对象的中间件
// expressJWT({secret:secretKey}是用来解析token的中间件
// unless({ path: [/^\/api\//] })用来解析哪些接口不需要访问权限
...
app.use((err, req, res, next) => {
  ...
  res.send({ status: 500, message: '其他未知错误！' })
})
